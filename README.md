...
- 👀 **20-20-20 护眼** —— 每 20 分钟弹出提醒卡「看远处 20 秒」：**不休息就一直在**，按 ⌥⌘B 才算休息并开始下一轮；忙就点「本次只眨眼休息」顺延 5 分钟。临近滴药时自动合并成「顺便滴药」提醒。该法则有 2023 年临床研究支持（[文献](https://pubmed.ncbi.nlm.nih.gov/35963776/)）。
- 🤫 **会议静音 / 离席暂停** —— `⌥⌘M` 开会静音、`⌥⌘S` 人不在电脑前暂停，全部手动恢复。
- 💊 **滴药即休息** —— 滴完眼药水自动刷新护眼计时；临近滴药的护眼提醒自动合并，不折腾。
- 🌙 **熄屏即休息** —— 合盖/熄屏超过 2 分钟自动清空护眼计时，用药按作息时间点不漂移。
- 📊 **休息周报** —— 每周六上午自动生成上周休息报告（总时长/主动休息/滴药/每日分布/环比），面板随时可看。
...
| <kbd>⌥</kbd> <kbd>⌘</kbd> <kbd>S</kbd> | 暂停 / 继续（人不在电脑前时用） |
| <kbd>⌥</kbd> <kbd>⌘</kbd> <kbd>P</kbd> | 唤出 / 隐藏面板 |
...
demo 模式：5 秒后弹用药确认窗（含震动+脉冲环+音效），15 秒后弹护眼确认卡，各自动效完整播放：
